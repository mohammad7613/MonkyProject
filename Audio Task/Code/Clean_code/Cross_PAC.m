...
if ~exist(fullfile(mat_files.folder,'Cross_PAC1'), 'dir')
        mkdir(fullfile(mat_files.folder,'Cross_PAC1'));
end
...
Fs = 1000;
pac =  nan(numberchannel,numberchannel,numWindows,3); % total time = 1 sec(200ms window with 100ms overlap) 3= number of set
CI_1 =  nan(numberchannel,numberchannel,numWindows,3);
CI_2 =  nan(numberchannel,numberchannel,numWindows,3);
...
tf_MVL_all = zeros(numberchannel,numberchannel,amp_length,phase_length);

 for cnt = 1:numWindows
    idx = (cnt-1)*overlap+1;
    x = erp(:,idx:idx+windowSize-1);
    for cl = 1:numberchannel
        for ch = 1:numberchannel
        for k = 1:phase_length
            for j = 1:amp_length
                l_freq = lowfreq(k);
                h_freq = highfreq(j);
                % [tf_MVL_all(c,j,k)] = tfMVL(x(c,:), h_freq, l_freq, Fs);
                [tf_MVL_all(ch,cl,j,k)] = tfMVL2(x(ch,:), h_freq,x(cl,:), l_freq, Fs);
            end
        end
        end
    end
    ...
    tf_MVL_mean = mean(mean(tf,3),4); 
    pac(:,:,cnt,sets)=tf_MVL_mean;
    pac_v=reshape(tf, numberchannel,numberchannel,[]);
    trialnumber = length(pac_v(1,1,:));
    mean_sgn = mean(pac_v,3);
    std_sgn = std(pac_v,1,3)/sqrt(trialnumber);
 ...
 for cl = 1:numberchannel
     ...
     for sets= 1:3
% %         curve1 = CI_2(ch,cl,:,sets);
% %         curve2 = CI_1(ch,cl,:,sets);
% %         t2 = [t, fliplr(t)];
% %         inBetween = [curve1(:)', fliplr(curve2(:)')];
% %         fill(t2, inBetween, 'g','FaceAlpha',0.3);
% %         hold on;
        plot(t,reshape(pac(ch,cl,:,sets), [],1),'LineWidth',1.5);
        hold on;
     end
...
,', channel AMP: ',num2str(ch),', channel Phase: ',num2str(cl),'\hspace{0.2cm}',Epoch.set(1).events(event).name),'interpreter','latex','fontsize',16);
    ...
    saveas(gcf,strcat(mat_files.folder,'\Cross_PAC1\adynamic_pac',int2str(event),'_CH',int2str(ch),int2str(cl),'.png'))
    hold off;
