...
eyeDataFileName = fullfile(sessionDir, ...
    strcat(sessionDateTime, '-eye', '.csv'));
resultsFileName = fullfile(sessionDir, ...
    strcat(sessionDateTime, '-res', '.csv'));
...
rewTimer = tic;
eyeTimer = tic;% Timer for automatic rewards
...
for iBlock = 1:nBlock
    ...
    while taskState ~= FINISHING && ~Params.isStopped
        ...
        gazeLoc = pos{1};
        fprintf(eyeFd, '%d,%d,%d,%d\n',toc(eyeTimer),gazeLoc(1),gazeLoc(2),0);
        ...
        switch taskState
            ...
            case PRESENTING
                ...
                elseif ~Params.isPaused && ~isFixated
                    ...
                    tag = flip(decimalToBinaryVector(FAILURE_CODE, 8));
                    outputSingleScan(shTag, tag)
                    taskState = PUNISHING;
                elseif ~Params.isPaused && isFixated && ...
                    ...
                    tag = flip(decimalToBinaryVector(SUCCESS_CODE, 8));
                    outputSingleScan(shTag, tag);
                    taskState = RESTING;
