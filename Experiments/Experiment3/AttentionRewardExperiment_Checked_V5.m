...
duration.presentation = 1.5  - duration.triggerPulseWidth;
duration.fixation = 0.0;
...
NumberOfEyePositions = NumberEyePositionsPerTrial*nTrial;
EyeData = zeros(NumberOfEyePositions,3);
% for c=1:nTrial
...
%                sendtrig(sendTriggers, connectToEyeTracker, shTag, EYE_TRIGGER)
               ...
               if CountSampleEyeTracker<NumberEyePositions
                   EyeData(CountSampleEyeTracker,1) = x;
                   EyeData(CountSampleEyeTracker,2) = y;
                   EyeData(CountSampleEyeTracker,3) = toc(onsetTimer);
               end    
