...
%% Create Session Handler for Analog Data
shEyeTracker = daq.createSession('ni');
[chAO0, idxAO0] = addAnalogOutputChannel(shEyeTracker, ...
    'Dev1', 'ao0', 'Voltage');
[chAO1, idxAO1] = addAnalogOutputChannel(shEyeTracker, ...
    'Dev1', 'ao1', 'Voltage');
...
R = 300;
c = 1;
RewardDimention = randperm(3,1);
RewardFeature = randperm(4,1);
disp(['category =' num2str(RewardDimention) 'stimuus = ' num2str(RewardFeature)]);
    ...
    for j=1:feature_number
        for k=1:feature_number
            for angle=1:angle_state_num
                ...
                trials(c).Rewsituation = [RewardDimention, RewardFeature];
                c=c+1;
            end    
        end
    end
...
AttentionSpan = ang2pix(Params.fixwindow, monitorDistance, monitorWidth / screenWidth);% it should be defined in terms of eccentricity



% *determine the reward parameter
extendedRewardTime = 1;
normalRewardTime = 0.2;
backGroundRewardTime = 0.1;
FoucsTimeToGetReward = 0.5; %ms
...
try
    ...
    while taskState ~= FINISHING && ~Params.isStopped
        ...
        fixSize = ang2pix(15, monitorDistance, monitorWidth / screenWidth);    % in pixels
%         fixSize = screenHeight;
        ...
        %%%%%%%%%%%% send adata to ecording system %%%%%%%%%%%%%%%%%%%%
          loc = pos{1} - [960, 540]; % rebase the origin to screen center
          pupil_size = pos{2} / 2500 * 5;
          loc = (loc ./ [960, 540]) * 5; % convert from pixel to voltage, maximum of 5V
          loc(1) = max(min(loc(1), 5), -5); % saturates X, confining it to -+ 5V
          loc(2) = max(min(loc(2), 5), -5); % saturates Y, confining it to -+ 5V
          outputSingleScan(shEyeTracker, [loc(1),loc(2)]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                     Screen('Flip', winPtr);
                    ...
                    %M_change
                    %Screen('frameoval', winPtr, [0 0 0],CenterRect([0 0 fixSize.*2 fixSize.*2], winRect1));
                ...
                case REWARD
                    if (rand()<trials(iTrial).RewProb)
                        disp(['distance=' num2str(distanaceFromRewardDimention)]);
                        Params.manualReward = true;
                        disp(['timefix=' num2str(accummulatedFocusTime)]);
